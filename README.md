# learning-journey
私がサーバーセキュリティエンジニアになるまでの過程で何をしてきたかをここに残しています。


# My Learning Journey 🚀

高校2年生から、サーバーセキュリティエンジニアを目指して学習を始めました。
このリポジトリは、その学習記録です。

## 🎯 目標
- アメリカのコミュニティカレッジ → UC Berkeley編入
- サーバーセキュリティエンジニアとして働く
- 世界中の人々をサイバー攻撃から守る

## 📚 学習中の内容
- プログラミング（HTML/CSS, JavaScript, Python）
- コンピュータサイエンスの基礎
- サーバーセキュリティ
- 学術論文の読解
- 英語（TOEFL/IELTS準備）

## 📚 取得済み試験
- 英検準2級
-
-

## 📖 読んだ論文
- Bitcoin: [A Peer-to-Peer Electronic Cash System](papers/bitcoin-abstract.md)

## 💻 プログラミング
- Progate: [HTML/CSS, JavaScript, Python 完了](programming/progate-progress.md)
- 

## 📚 学習した教材
- 大岩のいちばんはじめの英文法
- ポラリス1,2
- 肘井学の読解のための英文法
- 関正生のTherulus英語長文1
- ビック・ファット・キャットの世界一簡単な英語の本
- 図解まるわかりコンピュータのしくみ
- IELTSブリティッシュ・カウンシル公認問題集
- IELTSスピーキング徹底攻略
ーーーーーーーーーーーーーーーーーーーー ↑ ここまでのはGithubをやってませんでした。
- [いちばんやさしいITパスポート](book/cs/it-passport)

## 🌐 SNS
- TikTok: [@zeroglobal0](https://tiktok.com/@zeroglobal0)

---

**Last Updated**: 2024年10月
